...
var (
	...
	checkPackage     = "github.com/blockopsnetwork/telescope/internal/component/all"
)
...
func declaresComponent(pkg *packages.Package, file *ast.File) bool {
	...
	for _, decl := range file.Decls {
		...
		ast.Inspect(funcDecl.Body, func(n ast.Node) bool {
			...
			// github.com/blockopsnetwork/telescope/component.
			if pkgName, ok := pkg.TypesInfo.Uses[ident].(*types.PkgName); ok {
				if pkgName.Imported().Path() == "github.com/blockopsnetwork/telescope/internal/component" &&
